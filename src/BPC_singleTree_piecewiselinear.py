...
class BPC_singleTree_piecewiselinear:
    ...
    def rmp_path(self, can, type):
        ...
        for a in self.network.links:
            ...
            for piecewise in range(0, num_pieces):
                
                xconv = a.C * max_capacity_mult/num_pieces * (piecewise)
                tx = a.getTravelTime(xconv, 'UE')
                txp = a.getDerivativeTravelTime(xconv)
                
                #for c in range(0, 10):
                #    axcheck = a.C * max_capacity_mult / 10 * c
                #    print("\tcheck", a.getTravelTime(axcheck, 'UE')*axcheck - axcheck * (xconv * txp + tx) - xconv * xconv * txp)
                
                rmp.add_constraint(rmp.mu[a] >= rmp.x[a] * (xconv * txp + tx) - xconv * xconv * txp)
        ...
        if rmp.solve_details.status == 'infeasible' or rmp.solve_details.status == 'integer infeasible':
            ...
            #for a in self.network.links:
                #ax = rmp.x[a].solution_value
                #mua = rmp.mu[a].solution_value
                #print("\t", a, a.getTravelTime(ax, 'UE')*ax-mua, ax, a.C)
    ...
    def BB(self):
        ...
        if self.params.PRINT_BB_INFO or self.params.PRINT_BB_BASIC:
            print('---BPC_singleTree_piecewiselinear---') 
        ...
        while conv == False:                    
            ...
            if status == 'infeasible':
                ...
                elif self.params.pruneRoundedSO:  
                    #print('--> cannot prune')
                        
                    roundedy = {}
                    for a in self.network.links2:
                        roundedy[a] = math.ceil(yCG[a])
                    if self.ydict.hasSO(roundedy) == True:
                        sotstt = self.ydict.getSO(roundedy)
                    else:
                        sotstt = self.network.tapas('SO',roundedy) 
                        self.ydict.insertSO(roundedy, sotstt)
                        self.OAcuts.append(self.getOAcut())
                        self.nSO += 1
                    if sotstt > self.UB:
                        prune = True
                        if self.params.PRINT_BB_INFO:
                            print('--> prune by SO check on ceil y')
