...
class FS_NETS_piecewiselinear:
    ...
    def milp_link(self,can):
        ...
        milp.mu = {a: milp.continuous_var(lb=0) for a in self.network.links}
        ...
        for a in self.network.links:
            #---OA cuts
            #milp.add_constraint(milp.mu[a] >= a.t_ff)
            for piecewise in range(0, num_pieces):
                ...
                milp.add_constraint(milp.mu[a] >= milp.x[a] * (xconv * txp + tx) - xconv * xconv * txp)
        ...
        milp.minimize(sum(milp.mu[a] for a in self.network.links))
