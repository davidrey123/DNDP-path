...
class BPC_twoPhase:
    ...
    def OABPC(self, can):
       ...
       n = BB_node.BB_node(self.network, 0, 0, can.LB, self.inf, [], [], False)
       #n.paths = can.paths
       OABPC_nodes = [n]
       nOABPC = 0
       UB_OABPC = self.UB_SO_DNDP
       ...
       while conv_OABPC == False:                    
           ...
           if runSO:
               ...
               #---update UB_OABPC                
               if can_OABPC.UB < UB_OABPC:
                   UB_OABPC = can_OABPC.UB
                   yOABPC = can_OABPC.y
                   ...
                   for n in OABPC_nodes:                    
                       if n.active == True and n.LB >= UB_OABPC:
                           n.active = False
           ...
           if len(candidates_OABPC) == 0:
               ...
               gap_OABPC = (UB_OABPC - LB_OABPC)/UB_OABPC                      
           
           if self.params.PRINT_BB_INFO or self.params.PRINT_BB_BASIC:
               print('OABPC> %d\t%d\t%.1f\t%.1f\t%.2f%%' % (nOABPC,self.nSO,LB_OABPC,UB_OABPC,100*gap_OABPC))
    ...
    def BB(self):
        ...
        print('To do:')
        print('Check that LBs are valid: why is possible that SO-TSTT > can.LB? (interdiction cuts?')
