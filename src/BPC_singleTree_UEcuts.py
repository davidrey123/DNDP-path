...
class BPC_singleTree_UEcuts:
    ...
    def getOAUEcut(self):
        ...
        for a in self.network.links:
            ...
            if a.y == 1:                
                cut['a'][a] = round(a.getTravelTime(a.x,'UE'), 3)
                cut['b'][a] = round(a.getPrimitiveTravelTime(a.x) - a.x * a.getTravelTime(a.x,'UE'), 3)
                print(a,cut['a'][a],cut['b'][a])
                          
            else:
                cut['a'][a] = 0.0
                cut['b'][a] = 0.0
                
        cut['OFV'] = self.network.getBeckmannOFV()
        print(cut['OFV'])
    ...
    def initOAcuts(self, can, nKNP):
        ...
        self.rt_TAP += time.time() - t0_TAP
        self.OAUEcuts.append(self.getOAUEcut()) 
        self.nUE += 1
        self.UB = can.UB
    ...
    def rmp_path(self, can, type):
        ...
        for OAUEcut in self.OAUEcuts:
            #---OA UE cuts
            rmp.add_constraint(OAUEcut['OFV'] >= sum(rmp.x[a]*OAUEcut['a'][a] + OAUEcut['b'][a] for a in self.network.links))            
        ...
        if self.params.PRINT_BB_INFO:
            print('nb of paths: %d, cplex time: %.1f, rmp time: %.1f, status: %s' % (len(self.getPaths()),rmp.solve_details.time,(time.time() - t0_RMP),rmp.solve_details.status))
    ...
    def BB(self):
        ...
        if self.params.PRINT_BB_INFO or self.params.PRINT_BB_BASIC:
            print('---BPC_singleTree with UE cuts---')
