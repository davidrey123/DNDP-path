...
class BC2:
        ...
        for a in self.network.links:
            ...
            if a.y == 1:
                ...
                if addOAcut == True:
                    ...
                    #---add OA cut to lp
                    self.milp.add_constraint(self.milp.mu[a] >= self.milp.x[a]*OAcut['a'] + OAcut['b'])
                    self.nOAcuts += 1
    ...
    def createMILP(self):        
        ...
        milp.smu = milp.continuous_var(lb=0,name='smu')
        ...
        milp.add_constraint(sum(milp.mu[a] for a in self.network.links) == milp.smu)
        milp.minimize(milp.smu)
    ...
    def OA_link(self,can):
        ...
        while conv_OA == False:
            ...
            if self.ydict.hasSO(yMILP) == True:
                ...
                t0_TAP = time.time()
                tstt = self.network.tapas('SO_OA_cuts',yMILP)              
