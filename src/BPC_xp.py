...
class BPC:
    ...
    def nodeSelection(self, type, candidates):
        ...
        if type == 'bestBound':
            ...
            min_LB_nodes = [n for n in candidates if n.LB<=(min_LB + self.INT_tol)]
    ...
    def solveRMP(self,can):
        ...
        if self.rmp.solve_details.status == 'infeasible' or self.rmp.solve_details.status == 'integer infeasible':
            ...
            yopt = {}
            for a in self.network.links2:
                yopt[a] = round(self.rmp.y[a].solution_value,self.params.rd)
                ...
                xtx = a.t_ff * self.rmp.x[a].solution_value + a.gtilde * maxscore              
                can.score[a.id] = self.rmp.x[a].solution_value * xtx
        ...
        if self.params.PRINT_BB_INFO:
            print('nvars: %d, ncons: %d, nOAcuts: %d, cplexTime: %.1f, RMPTime: %.1f' % (self.rmp.number_of_variables,self.rmp.number_of_constraints,self.nOA,self.rmp.solve_details.time,(time.time() - t0_RMP)))
    ...
    def BB(self):
        ...
        while conv == False:                    
            ...
            if self.params.PRINT_BB_INFO or self.params.PRINT_BB_BASIC:
                ...
                print('%d\t%d\t%d\t%d\t%.1f\t%.1f\t%.2f%%\t%.1f' % (self.nBB,npaths,self.nOA,self.nUE,self.LB,self.UB,100*self.gap,elapsed_time))
